...
const Planet = ({ name, position, onClick, isSelected, textures = {} }) => {
  ...
  useFrame((state, delta) => {
    if (meshRef.current) {
      meshRef.current.rotation.y += PLANET_CONFIG[name]?.rotationSpeed * delta * 100 || 0;
    }
  });
...
const SolarSystemScene = ({ selectedPlanet, onPlanetSelect, cameraState }) => {
  ...
  useFrame((state, delta) => {
    ...
    if (cameraState === 'earth') {
      state.camera.position.lerp(new THREE.Vector3(0, 0, 15), 0.05);
      if (controlsRef.current) controlsRef.current.target.lerp(new THREE.Vector3(25, 0, 0), 0.05);
    } else if (cameraState === 'solar') {
      state.camera.position.lerp(new THREE.Vector3(0, 30, 80), 0.05);
      if (controlsRef.current) controlsRef.current.target.lerp(new THREE.Vector3(0, 0, 0), 0.05);
    }
  });
...
const SolarSystem = () => {
  ...
  const [cameraState, setCameraState] = useState('solar');

  return (
    <div className="w-full h-screen">
      <Canvas camera={{ position: [0, 20, 80], fov: 60 }} gl={{ antialias: true, alpha: true }}>
        <SolarSystemScene selectedPlanet={selectedPlanet} onPlanetSelect={setSelectedPlanet} cameraState={cameraState} />
      </Canvas>
      {selectedPlanet && (
